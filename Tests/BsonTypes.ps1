...
$(
	...
	$m.binData1 = $v
	equals $m.binData1 $v
	Test-Data binData1 ([guid])
	Test-Bson binData1 ([MongoDB.Bson.BsonBinaryData])

	# bytes - one way byte[] -> BsonBinaryData
	$v = [byte[]](1, 2)
	$m.binData2 = $v #_191108_183844
	equals $m.binData2 $m.binData2
	Test-Data binData2 ([MongoDB.Bson.BsonBinaryData])
	Test-Bson binData2 ([MongoDB.Bson.BsonBinaryData])

	# ReferenceEquals()
	assert ([object]::ReferenceEquals($m.binData2, $m.binData2))

	# the original byte[] instance
	assert ([object]::ReferenceEquals($m.binData2.Bytes, $v))
)
